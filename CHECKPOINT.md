...
| Escrita 3 · pedido pra equipe ✅ (2026-09-26) | `aurora_pedido_equipe` (MCP write 0.4.0, plugin carimbo 0.4.0, Sonoramente `20260926150000`, Central `2607d94`): desconto, financeiro, agenda, cadastro, clínico (→ responsável técnica), falar com alguém. Um cartão por pessoa+assunto+dia na lista de avisos. Nomes da equipe tirados também das descrições das ferramentas (read 0.5.1). Release `16cf348` na VPS, hash conferido; smoke com o GPT-6 Sol chamou a ferramenta. |
| Benchmark de modelos ✅ (2026-09-26) | 8 modelos × 10 perguntas reais (`docs/bench/2026-09-26-modelos.png`). Só os GPT-6 (assinatura Codex) fizeram 10/10 sem citar nomes e com respostas curtas; Sol high ~23 s pelo terminal. DeepSeek 4.1 Flash (atual) 8 certas + 2 leves (nomes) e não usou o pedido pra equipe no smoke. Muse Spark contributor fora (treina com os dados). Rodada 2 (`docs/bench/2026-09-26-modelos-rodada2.png`): Sol medium Fast 10/10 ~25 s (recomendado); Sol high 9+1 leve; Luna high 2 erros (disse "ficou registrado" com a ação bloqueada, e inventou consulta). Troca de modelo aguarda o Alf. |
| Modelo GPT-6 Sol medium Fast ✅ (2026-09-26 16:00 UTC) | `openai-codex/gpt-6-sol`, `reasoning_effort: medium`, `service_tier: fast`; reserva `opencode-go/deepseek-v4.1-flash` (testada com a config nova). Escolhido no benchmark rodada 2. |
| Serjão no privado + foto/PDF + treino ✅ (2026-09-26) | Número do Serjão liberado em `aurora_canal_config` (conversa ainda não existe: nasce quando ele escrever). Ponte 0.4.0 (Sonoramente `20260926161000`): foto da UAZAPI vai como imagem para o modelo; PDF é baixado, vira texto (`pdftotext`, até 10 páginas) na pasta de cache do Hermes e some em 24 h; só host `lamusic.uazapi.com`. Skill `copiloto-e-treino`: copiloto ("o que eu respondo?") e treino nos dois sentidos (ela cliente / ele cliente, ações simuladas). Release `79c78f6`, hash conferido; PDF real testado. Falta: teste de foto pelo WhatsApp. |
| Foto testada ✅ + ponte 0.4.1 (2026-09-26) | A Central grava a mensagem antes de baixar a mídia; a ponte agora espera a URL até 20 s (`aurora_ponte_midia`, Sonoramente `20260926163000`). Foto do Alf lida de ponta a ponta; PDF real passou. |
| Aurora Assistant ✅ (2026-09-26) | Central `ecb0ec7` (no ar em sonoramente-la.vercel.app): aba **Aurora** no lugar do AI Assist antigo (Gemini), com a sugestão da Aurora de verdade para a última mensagem e **Enviar / Editar / Copiar / Ignorar** (só admin e recepção, por RPC `aurora_assistente_sugestao`/`aurora_assistente_usar`). Uso vira régua: enviada = ok, editada = leve (guarda o texto final), copiada/ignorada = revisão. Ponte 0.5.0: com humano atendendo, em vez de calar ela entra em **modo sugestão** por 5 min (resposta vira `sugestao`, sem "digitando", escrita travada) e recebe o que a equipe respondeu (`respostas_equipe`). Sonoramente `20260926170000`, release `190192e`. Placar diário conta sugestões. |
| Áudio + botão único ✅ (2026-09-26 17:00 UTC) | Ponte 0.5.1 espera a transcrição do áudio até 30 s (`aurora_ponte_midia` devolve `transcricao`, Sonoramente `20260926171500`); antes a Aurora recebia só "🎤 Audio". Webhook da Central: download/transcrição com `EdgeRuntime.waitUntil` + 1 nova tentativa (1 em 19 áudios ficava sem arquivo). Aurora Assistant passou para o botão "Aurora" original (com avatar); aba duplicada removida. Central `da8546d`, release `7978bf6`. Falta: áudio real depois do deploy. |
| Áudio provado ✅ + botões de tom (2026-09-26 17:10 UTC) | Áudio do Alf transcrito e respondido pela Aurora (prova também o webhook novo com tráfego real). Aurora Assistant ganhou **Mais curto / direto / acolhedor / formal / detalhado** pela edge function `aurora-reescrever` (mesmo padrão do `reescrever-evolucao`, só admin/recepção, só muda a forma). Enviar depois de ajustar o tom = `ajustada` (conta como ok). "Atualizar" com retorno visual. Central `d770f2a`, Sonoramente `20260926173000`. |
| Botões de tom corrigidos na raiz ✅ (2026-09-26 17:40 UTC) | O Alf achou texto cortado e raciocínio do modelo vazando ("Let's use 🎵…"). Causa: raciocínio do Gemini dentro do limite de 1024 tokens. `aurora-reescrever` com raciocínio mínimo, filtro de partes `thought`, só `STOP`, trava de fatos (data/hora/valor) e chamada interna para teste. Bateria `scripts/testar-aurora-reescrever.py` 15/15 na função publicada. Sonoramente `f041a21`. |
| Cérebro único ✅ (2026-09-26 18:05 UTC) | `aurora-cerebro` na VPS (serviço `aurora-cerebro`, porta 8681, `runtime/cerebro/`): GPT-6 Sol pela assinatura, só texto→texto, token em `/home/aurora/.hermes/cerebro.token` (600), 60/min, sem conteúdo em log. Túnel `aurora-cerebro.maestrosdagestao.com.br` (mesmo túnel da Julia/Instagram; backup do config; os 3 endereços 200 depois). Token no Vault (`aurora_cerebro_token`, lido só por service_role via `aurora_cerebro_token()`, Sonoramente `20260926180000`), porque segredo de função exige Owner. `aurora-reescrever` usa o cérebro e cai no Gemini se a VPS falhar. Baterias na função publicada: GPT 15/15 (3,7 s), Gemini 15/15 (1,7 s), queda simulada 15/15 no Gemini, recusas ok. Próximos candidatos: "família" da evolução e resumos. |
