       ...
       (SELECT count(*) FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
         WHERE n.nspname = 'public'
           AND c.relname IN ('pacientes','responsaveis','paciente_responsavel','sessoes','cobrancas','leads',
                             'evolucoes','anamneses','aurora_equipe','aurora_identificadores','wa_mensagens')
...
function assertBadge(row, config) {
  if (
    ...
    throw new AdapterError('CREDENTIAL_SCOPE_INVALID', 'O crachá da Aurora não está restrito à leitura por RPC.');
  }
}
