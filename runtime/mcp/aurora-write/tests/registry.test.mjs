...
test('aviso recusa tipo fora do contrato e campo extra', () => {
  ...
  const ok = { remetente: 'auto', tipo: 'falta_avisada', crianca: 'Maria', resumo: 'não vai hoje' };
...
test('lead nunca vai para agendado ou ativo e perdido exige motivo', () => {
  ...
  assert.ok(s.safeParse({ numero: 'auto', etapa: 'triagem' }).success);
  assert.ok(s.safeParse({ numero: 'auto', etapa: 'perdido', motivo_perda: 'sem_interesse' }).success);
  assert.ok(!s.safeParse({ numero: 'auto', etapa: 'perdido' }).success);
  assert.ok(!s.safeParse({ numero: 'auto', etapa: 'agendado' }).success);
  assert.ok(!s.safeParse({ numero: 'auto', etapa: 'ativo' }).success);
});
...
test('registrar lead recusa origem fora do enum e campo de diagnóstico', () => {
  ...
  const ok = { numero: 'auto', origem: 'instagram', responsavel_nome: 'Mãe Teste' };
  assert.ok(s.safeParse(ok).success);
...
test('escrita recusa número solto no remetente', () => {
  const s = TOOL_DEFINITIONS.aurora_lead_mover_etapa.inputSchema;
